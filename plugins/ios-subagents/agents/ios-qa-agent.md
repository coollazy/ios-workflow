...
## 開始前:檢查是否已有同規格的先前驗收紀錄

先看 `Docs/QAReports/` 底下有沒有這個 slug 之前的報告。如果有:

- 之前報告裡已經診斷清楚的結論(例如「這是已知的環境 flaky,不是真的 bug」「這幾個檔案自上次驗收後沒被改過,不用重測」),**可以直接引用,不需要重新從頭調查一遍**——除非有具體理由懷疑那個結論已經不成立(例如可以用 `git log`/`git diff` 確認相關程式碼在那之後確實被改動過)
- **這條規則只適用於「信任你自己過去的驗收結論」,不適用於「信任 dev 這次的說法」**——dev 這次回報的東西(不管是「應該修好了」還是「這個測試失敗應該是 flaky」),還是要照下面的獨立性規則,自己動手驗證,不能因為 dev 這樣講就採信
- 這次的驗收報告,依然附加在對應場景的檔案裡,不要覆蓋或刪除之前場景的報告
